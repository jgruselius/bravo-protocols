...
<Velocity11 file='Protocol_Data' md5sum='dc0450b2b8b991ba7cf73212a2a49320' version='2.0' >
...
/*  Description of tip usage

There are 2 &apos;sets&apos; of tips and they&apos;re used as follows:
Subprocess, Set, process name, description

Transfer Block Mix, Set1, tips1, left side of head, right side of tip box
Mix DNA and Blockers, Set2, tips2, multiple columns
Transfer Capture Library,Set1, tips3, right side of head, left side of tip box
Transfer Hyb buffer, Set2, tips4, right side of head, left side of tip box, can use multiple columns
Transfer DNA,Set2, tips5, multiple columns

The above uses the tiptracking.js functions and script within the subprocesses
to manage tips.
*/' />
				</Task>
		...
		<Main_Processes >
			<Process >
				...
				<Task Name='BuiltIn::Spawn Process' >
					...
					<Has_Breakpoint >0</Has_Breakpoint>
					<Advanced_Settings />
					<TaskScript Name='TaskScript' Value=' var temp;

//if all tips are used, need to get full box from stacker

if (isEmpty())
   {
    print(plate.name+&quot;:  Spawning new tips from stacker&quot;);
   ...
   {
    task.skip();
   print(plate.name+&quot;:  Skipping spawn of TipControl.&quot;);
   }' />
					<Parameters >
...
if (!isEmpty())
   {
    print(plate.name+&quot;:   Spawning unload from minihub cassette 4 slot 1&quot;);
    temp = 1;
   }
else
   {
    task.skip();
   print(plate.name+&quot;:   Skipping spawn of unload from minihub cassette 4 slot 1&quot;);
   }' />
					<Parameters >
    ...
    print(&quot;Skipping spawn of tips5.&quot;)
			...
			<Process >
				...
				<Task Name='BuiltIn::Load' >
					...
					<TaskScript Name='TaskScript' Value='//if there are tips remaining, tip box will be used in 
//transfer hyb buffer

if (!isEmpty())
   {
    task.loadIntoByLocation = [&quot;Cassette4Slot1&quot;]
    print(plate.name + &quot;:  Placing at minihub &quot;+task.loadIntoByLocation);
   }
else
   {
...
' />
			...
			<Process >
				...
				<Task Name='BuiltIn::Load' >
					...
					<TaskScript Name='TaskScript' Value='//if used tip box is not full, it goes into minihub for transfer
//hyb buffer

if(!isFull())
   {
    print(plate.name+&quot;:  Load into minihub.&quot;)
   }
else
   {
    task.skip();
    print(plate.name+&quot;:  Skipping load into minihub.&quot;)
   }' />
					<Parameters >
						<Parameter Category='' Name='loadIntoByLocation' Value='' />
						<Parameter Category='' Name='loadIntoByGroup' Value='' />
    ...
    print(&quot;Skipping upstack of &quot; + plate.name + &quot;.&quot;)
			...
			<Process >
				...
				<Task Name='BuiltIn::Unload' >
					...
					<TaskScript Name='TaskScript' Value='if(temp)
{
   print(plate.name+&quot;:  Unloading from minihub cassette 4 slot 1.&quot;)
}
else
{
   task.skip();
   print(plate.name+&quot;:  Skipping unload from minihub cassette 4 slot 1.&quot;)
}' />
					<Parameters >
						<Parameter Category='' Name='unloadFrom' Value='' />
					</Parameters>
...
&lt;/Velocity11&gt;' />
				...
				<Task Name='BuiltIn::JavaScript' >
					...
					<TaskScript Name='TaskScript' Value='//using single column of tips for all of blocker transfer
//tips for Capture library transfer will come from this tip box too

var single_col_tips = 12
' />
				</Task>
				<Task Name='Bravo::secondary::Tips On' Task_Type='16' >
					<Enable_Backup >0</Enable_Backup>
					...
					<TaskScript Name='TaskScript' Value='task.Wellselection = [[ 1 , single_col_tips ]]

print(&quot;Pressing tips on at &quot; + task.Wellselection)

/*
tips_on_loc = tips_on_loc - 1
task.Wellselection = [[ 1 , tips_on_loc ]]
...
&lt;/Velocity11&gt;' />
				...
				<Task Name='Bravo::secondary::Tips On' Task_Type='16' >
					...
					<TaskScript Name='TaskScript' Value='//change column of tips to 1 for other side of tip box
single_col_tips = 1

task.Wellselection = [[ 1 , single_col_tips ]]
...
//need to use Set 2 tips starting from column 1 in tip box
//but still using tip tracking functions in .js file
var tips_on_loc_hyb = 0
var tips_off_loc_hyb = 0' />
					<Parameters >
						<Parameter Category='' Name='Head mode' Value='&lt;?xml version=&apos;1.0&apos; encoding=&apos;ASCII&apos; ?&gt;
&lt;Velocity11 file=&apos;MetaData&apos; md5sum=&apos;d4b0ccac7e9c488ef9b03e8cebe75927&apos; version=&apos;1.0&apos; &gt;
	&lt;PipetteHeadMode Channels=&apos;0&apos; ColumnCount=&apos;1&apos; RowCount=&apos;8&apos; SubsetConfig=&apos;0&apos; SubsetType=&apos;1&apos; TipType=&apos;0&apos; /&gt;
&lt;/Velocity11&gt;' />
				...
				<Task Name='Bravo::secondary::Tips On' Task_Type='16' >
					...
					<TaskScript Name='TaskScript' Value='tips_on_loc_hyb = tips_on_loc_hyb + 1
task.Wellselection = [[ 1 , tips_on_loc_hyb ]]
...
&lt;/Velocity11&gt;' />
				...
				<Task Name='Bravo::secondary::Tips Off' Task_Type='32' >
					...
					<TaskScript Name='TaskScript' Value='tips_off_loc_hyb = tips_off_loc_hyb + 1
task.Wellselection = [[ 1 , tips_off_loc_hyb ]]
print(&quot;Taking tips off at &quot; + task.Wellselection)
' />
					<Parameters >
...
if(remaining_new_tips &gt; 0)
   {
    ...
    task.Body = &quot;There are still &quot; + remaining_new_tips + &quot; columns of unused tips in the tip box at Bravo Position 2.  There are also new tips in cassette 1, slot 1.&quot;
   }
else
   {
    task.Title = &quot;Empty Tip box&quot;
    task.Body = &quot;The tip box at Bravo Position 2 is empty and can be used in other protocols.  There are also new tips in cassette 1, slot 1.&quot;
   }
' />
					<Parameters >
