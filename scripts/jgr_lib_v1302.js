...
/*
 CHANGED in v1302:
 --Fixed bug in parseDilutionTransfers where newTip was set to false if
   buffer volume of first transfer was skipped
 --Sort transfers by plate then by well to avoid unexpected transfer order
 --Allow strings as plate id's as well as indexes
 --Check for same-volume transfers
 TODOs:
 --Enable transfer using another headmode -> Re-define the Tipbox class
...
function Transfer(sourcePlate, sourceWell, volume, destinationWell, newTip) {
	...
	this.toString = function () {
		var s = ["{sourcePlate: " + this.sourcePlate,
...
function parseTransfers(str) {
	...
	for(var i = 0, n = rowArray.length; i < n; i++) {
		...
		try {
			...
			/* Assign an index to the plate ID (is this needed??):
			if(sourcePlate in plateMap) {
				sourcePlateIndex = plateMap[sourcePlate];
			} else {
				sourcePlateIndex = Object.keys(plateMap).length;
